  ...
  .controller('dictionaryCtrl', ['$scope', 'ngTableParams', 'SpecofferDictionaryService',
    function ($scope, NgTableParams, SpecofferDictionaryService) {
      ...
      $scope.dictionaries = [
        ...
        {name: 'Дані про пільги', headers: {benefitTypeId: 'Тип пільги', abbrName: 'Скорочена назва', name: 'Назва пільги', identifier: 'Інформація про коди пільг вступу', begDate: 'Дата початку переваги',
          endDate: 'Дата закінчення переваги', description: 'Опис', parentId: 'Ієрархічний ідентифікатор'}, id:8, dict: function() {SpecofferDictionaryService.getBenefits().then(function (data){$scope.newData = data;})}},
        ...
        {name: 'Адміністративно-територіальні одиниці', headers: {adminUnitTypeId: 'Ідентифікатор типу адміністративно-територіальної одиниці',
          identifier: 'Ідентифікатор КОАТУУ адміністративно-територіальної одиниці',
          name: 'Назва адміністративно-територіальної одиниці',
          fullName: 'Повна назва адміністративно-територіальної одиниці',
          identifier1: 'Обласний ідентифікатор КОАТУУ адміністративно-територіальних одиниць',
          identifier2: 'Районний ідентифікатор КОАТУУ адміністративно-територіальних одиниць',
          identifier3: 'Місцевий ідентифікатор КОАТУУ адміністративно-територіальних одиниць',
          begDate: 'Дата початку', endDate: 'Дата кінця',
          parentId: 'Ієрархічний ідентифікатор'}, id:13, dict: function() {SpecofferDictionaryService.getAdminUnits().then(function (data){$scope.newData = data;})}},
        ...
        {name: 'Нагороди за публічні заходи', headers: {publicActivityId: 'Ідинтифікатор публічної активаності', enrolmentSubjectId: 'Ідентифікатор дисципліни ЗНО абітурієнта', awardName: 'назва нагороди публічної активності', bonus: 'Бонус додаткові бали за публічну активність', begDate: 'Дата початку', endDate: 'Дата закінчення'}, id:17, dict: function() {SpecofferDictionaryService.getPublicActivitiesAwards().then(function (data){$scope.newData = data;})}},
        {name: 'Публічні активності', headers: {publicActivityTypeId: 'Індитифікатор типу публічної активності', timePeriodId: 'Ідинтифікатор часового періоду', name: 'Назва активності', begdate: 'Дата початку', enddate: 'Дата закінчення'}, id:18, dict: function() {SpecofferDictionaryService.getPublicActivities().then(function (data){$scope.newData = data;})}},
        {name: 'Довідник придметів ЗНО', headers: {abbrName: 'Скорочена назва', name: 'Назва предмету ЗНО', isTesting: 'Чи є предмет в списку предметів ЗНО', parentId: 'Iєрархічний ідентифікатор'}, id:19, dict: function() {SpecofferDictionaryService.getEnrolmentsSubjects().then(function (data){$scope.newData = data;})}},
        ...
        {name: 'Довідник типів документів', headers: {abbrName: 'Скорочена назва', name: 'Тип документу', paperUsageId: 'Ідинтифікатор типу використання документу'}, id:22, dict: function() {SpecofferDictionaryService.getPaperTypes().then(function (data){$scope.newData = data;})}},
        {name: 'Довідник типів поступлень', headers: {abbrName: 'Скорочена назва', name: 'Назва поступлення', parentId: 'Iєрархічний ідентифікатор'}, id:23, dict: function() {SpecofferDictionaryService.getEnrolmentsTypes().then(function (data){$scope.newData = data;})}},
        {name: 'Довідник типів статусу заявки', headers: {abbrName: 'Скорочена назва', name: 'Тип статусу', description: 'Опис'}, id:24, dict: function() {SpecofferDictionaryService.getEnrolmentsStatusTypes().then(function (data){$scope.newData = data;})}}
      ];
