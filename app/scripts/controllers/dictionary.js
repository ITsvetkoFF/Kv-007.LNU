...
angular.module('admissionSystemApp')
  .controller('dictionaryCtrl', ['$scope', 'ngTableParams', 'DictionariesSvc',
    function ($scope, NgTableParams, DictionariesSvc) {
      ...
      $scope.dictionaries = [
        ...
        ], id:1, dict: function() {DictionariesSvc.getTimeperiodsTypes().then(function (data){$scope.newData = data;});}},
        {name: 'Довідник типів спеціальностей', headers: [
          {dictKey: 'abbrName', dictHeader: 'Скорочена назва'},
          {dictKey: 'name', dictHeader: 'Тип спеціальності'}
        ], id:2, dict: function() {DictionariesSvc.getSpecialtiesTypes().then(function (data){$scope.newData = data;});}},
        ...
        ], id:3, dict: function() {DictionariesSvc.getAllSpecialties().then(function (data){$scope.newData = data;});}},
        ...
        ], id:5, dict: function() {DictionariesSvc.getSpecoffersTypes().then(function (data){$scope.newData = data;});}},
        {name: 'Довідник форм навчання', headers: [
          {dictKey: 'abbrName', dictHeader: 'Скорочена назва'},
          {dictKey: 'name', dictHeader: 'Тип форми навчання'}
        ], id:6, dict: function() {DictionariesSvc.getEduformTypes().then(function (data){$scope.newData = data;});}},
        {name: 'Довідник типів пільг', headers: [
          {dictKey: 'abbrName', dictHeader: 'Скорочена назва'},
          {dictKey: 'name', dictHeader: 'Тип пільги'},
          {dictKey: 'priority', dictHeader: 'Квота (1 - квота, 0 - пільга)'},
          {dictKey: 'parentId', dictHeader: 'Ієрархічний ідентифікатор'}
        ], id:7, dict: function() {DictionariesSvc.getBenefitsTypes().then(function (data){$scope.newData = data;});}},
        {name: 'Дані про пільги', headers: [
          ...
          {dictKey: 'parentId', dictHeader: 'Ієрархічний ідентифікатор'}
        ], id:8, dict: function() {DictionariesSvc.getBenefits().then(function (data){$scope.newData = data;});}},
        {name: 'Довідник типу персон', headers: [
          {dictKey: 'abbrName', dictHeader: 'Скорочена назва'},
          {dictKey: 'name', dictHeader: 'Тип персони'}
        ], id: 9, dict: function() {DictionariesSvc.getPersonsTypes().then(function (data){$scope.newData = data;});}},
        {name: 'Довідник статі персон', headers: [
          {dictKey: 'abbrName', dictHeader: 'Скорочена назва'},
          {dictKey: 'name', dictHeader: 'Стать персони'}
        ], id: 10, dict: function() {DictionariesSvc.getGenderTypes().then(function (data){$scope.newData = data;});}},
        {name: 'Довідник сімейного стану', headers: [
          {dictKey: 'abbrName', dictHeader: 'Скорочена назва'},
          {dictKey: 'name', dictHeader: 'Тип персони'}
        ], id:11, dict: function() {DictionariesSvc.getMarriedTypes().then(function (data){$scope.newData = data;});}},
        ...
        ], id:12, dict: function() {DictionariesSvc.getAdminUnitsTypes().then(function (data){$scope.newData = data;});}},
        {name: 'Адміністративно-територіальні одиниці', headers: [
          ...
          {dictKey: 'parentId', dictHeader: 'Ієрархічний ідентифікатор'}
        ], id: 13, dict: function() {DictionariesSvc.getAdminUnits().then(function (data){$scope.newData = data;});}},
        {name: 'Довідник типу адрес', headers: [
          {dictKey: 'abbrName', dictHeader: 'Скорочена назва'},
          {dictKey: 'name', dictHeader: 'Тип адреси'}
        ], id: 14, dict: function() {DictionariesSvc.getAddressTypes().then(function (data){$scope.newData = data;});}},
        {name: 'Довідник типів контактів', headers: [
          {dictKey: 'abbrName', dictHeader: 'Скорочена назва'},
          {dictKey: 'name', dictHeader: 'Тип контакту'}
        ], id: 15, dict: function() {DictionariesSvc.getContactsTypes().then(function (data){$scope.newData = data;});}},
        {name: 'Довідник типу публічних активностей', headers: [
          {dictKey: 'abbrName', dictHeader: 'Скорочена назва'},
          {dictKey: 'name', dictHeader: 'Тип публічної активності'}
          ], id: 16, dict: function() {DictionariesSvc.getPublicActivitiesTypes().then(function (data){$scope.newData = data;});}},
        ...
        ], id: 17, dict: function() {DictionariesSvc.getPublicActivitiesAwards().then(function (data){$scope.newData = data;});}},
        {name: 'Публічні активності', headers: [
          ...
          ], id:18, dict: function() {DictionariesSvc.getPublicActivities().then(function (data){$scope.newData = data;});}},
        {name: 'Довідник придметів ЗНО', headers: [
          ...
          ], id:19, dict: function() {DictionariesSvc.getEnrolmentsSubjects().then(function (data){$scope.newData = data;});}},
        {name: 'Довідник типів відзнак', headers: [
          {dictKey: 'abbrName', dictHeader: 'Скорочена назва'},
          {dictKey: 'name', dictHeader: 'Тип відзнаки'}
          ], id:20, dict: function() {DictionariesSvc.getHonorsTypes().then(function (data){$scope.newData = data;});}},
        {name: 'Довідник типів використання документів', headers: [
          {dictKey: 'abbrName', dictHeader: 'Скорочена назва'},
          {dictKey: 'name', dictHeader: 'Тип використання документу'}
          ], id:21, dict: function() {DictionariesSvc.getPaperUsages().then(function (data){$scope.newData = data;});}},
        {name: 'Довідник типів документів', headers: [
          ...
          ], id:22, dict: function() {DictionariesSvc.getPaperTypes().then(function (data){$scope.newData = data;});}},
        {name: 'Довідник типів поступлень', headers: [
          {dictKey: 'abbrName', dictHeader: 'Скорочена назва'},
          {dictKey: 'name', dictHeader: 'Назва поступлення'},
          {dictKey: 'parentId', dictHeader: 'Iєрархічний ідентифікатор'}
          ], id:23, dict: function() {DictionariesSvc.getEnrolmentsTypes().then(function (data){$scope.newData = data;});}},
        {name: 'Довідник типів статусу заявки', headers: [
          {dictKey: 'abbrName', dictHeader: 'Скорочена назва'},
          {dictKey: 'name', dictHeader: 'Тип статусу'},
          {dictKey: 'description', dictHeader: 'Опис'}
          ], id:24, dict: function() {DictionariesSvc.getEnrolmentsStatusTypes().then(function (data){$scope.newData = data;});}}
      ];
